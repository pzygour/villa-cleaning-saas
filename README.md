...
## Phase 7 security/runtime hardening

- CSRF protection is required for all state-changing API requests (`POST`, `PUT`, `PATCH`, `DELETE`).
  - Admin pages inject a session CSRF token into `window.CSRF_TOKEN` and automatically attach it as `X-CSRF-Token` on mutating `fetch` calls.
- Session hardening includes strict session mode, HTTP-only cookies, SameSite=Lax cookies, and session ID regeneration after successful login.
- Idle session timeout is controlled via `SESSION_TIMEOUT_SECONDS` (default `3600`).
- Public API error responses remain sanitized; enable debug verbosity with `APP_DEBUG=1` only in non-production environments.

Environment knobs in `config/app.php`:
- `APP_ENV` (default `production`)
- `APP_DEBUG` (`0|1`, default `0`)
- `APP_TIMEZONE` (default `UTC`)
- `SESSION_TIMEOUT_SECONDS` (default `3600`)


## Phase 8 launch-readiness safeguards

- Login throttling is configurable via:
  - `LOGIN_MAX_ATTEMPTS` (default `5`)
  - `LOGIN_LOCKOUT_SECONDS` (default `300`)
- Security events are logged for: `login_success`, `login_failed`, `login_throttled`, `logout`, `session_expired`, `csrf_invalid`, and `forbidden_access`.
- Use `docs/launch-readiness-checklist.md` before production launch for QA smoke verification and deployment checks.
