...
function render_admin_page(string $title, string $contentHtml, array $scripts = []): void
{
    ...
    echo 'window.CSRF_TOKEN=' . json_encode($csrfToken, JSON_THROW_ON_ERROR) . ';';
    ...
    echo '(function(){const _fetch=window.fetch.bind(window);window.fetch=function(input,init){const options=init||{};if(typeof input==="string"&&input.startsWith("/")&&!input.startsWith(window.APP_BASE_PATH+"/")){input=window.APP_BASE_PATH+input;}const method=String(options.method||"GET").toUpperCase();if(["POST","PUT","PATCH","DELETE"].includes(method)){options.headers=options.headers||{};if(!options.headers["X-CSRF-Token"]&&!options.headers["x-csrf-token"]){options.headers["X-CSRF-Token"]=window.CSRF_TOKEN||"";}}return _fetch(input,options);};})();';
    echo '</script>';
